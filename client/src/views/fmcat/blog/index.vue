    ...
    <div class="color-block vertical table-container">
      <el-table ref="multipleTable" :data="blogList" tooltip-effect="dark"
        ...
        <el-table-column label="标签" min-width="300">
          <template slot-scope="scope">
            <el-tag size="mini" disable-transitions v-for="item in scope.row.tags_text.split(',')"
              :key="item">{{item}}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="150">
          <template slot-scope="scope">
            <el-button @click="$router.push('/fmcat/blog/detail/'+scope.row.id)"
              type="text" size="small">查看</el-button>
            <el-button @click="$router.push('/fmcat/blog/edit/'+scope.row.id)"
              type="text" size="small">编辑</el-button>
            <el-button @click="blogDelete(scope.row.id,scope.row.class_id)"
              type="text" size="small">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div style="margin-top: 20px">
        <el-button size="small" :disabled="checkSelectEmpty" @click="setTop"
          type="success">置顶</el-button>
        <el-button size="small" :disabled="checkSelectEmpty" @click="setDisabled"
          type="info">下架</el-button>
        <el-button size="small" :disabled="checkSelectEmpty" @click="deleteSelect"
          type="danger">删除</el-button>
      </div>
      <div class="page-box">
        <el-pagination layout="prev, pager, next" :total="blogCount" :page-size="10"
          :current-page="currentPage" @current-change="pageChange" background>
        </el-pagination>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    setTop() {
      ...
      let that = this
      that
        .$confirm(`此操作将置顶这${ids.length}篇博客, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
    ...
    setDisabled() {
      ...
      let that = this
      that
        .$confirm(`此操作将下架这${ids.length}篇博客, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
